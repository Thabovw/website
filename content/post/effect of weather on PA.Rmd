...
While I'm a writing this blog, I'm sitting in the train. Soaked... again.
It has been raining for quite some weeks now and the weather report also not very hopeful. Of course this weather doesn't make me happy and I certainly don't want to go outside to train for the half-marathon. To make myself a little less depressed, I think it is time for a new blog. 

In this blog, I wanted to see whether I am the only one, or whether the kids in the [MyMovez project](https://bmcpublichealth.biomedcentral.com/articles/10.1186/s12889-018-5353-5#Sec4) sample also are less physically active, and whether they are less happy on rainy days.
...
But before we start, first let is get in the mood by playing the sound on [this website](https://rainymood.com/).

The data of the project is publicly available, [clickettyclickclick](https://easy.dans.knaw.nl/ui/datasets/id/easy-dataset:155345). In addition, the KNMI (Royal Netherlands Meteorological Institute) shares the day-to-day weather data on [their website](http://projects.knmi.nl/klimatologie/daggegevens/selectie.cgi). For a conference poster, I downloaded the data from 27-01-2016 until 04-07-2018. This data is already merged to the physical activity data, so you don't have to do this yourself.
...
I want to make two new variables. One that informs us what year it is, the other the day number in that year. For example, 4th of July in 2018 is in the year 2018 (duh) and the 4th of July is the ...

```{r, echo=TRUE}
lubridate::yday("2018-07-04")
```

... 185th day of the year of course. I knew that. this function is not on the [cheatsheet](https://evoldyn.gitlab.io/evomics-2018/ref-sheets/R_lubridate.pdf), but believe me, it works.

But yeah, we use the same function for all the dates:
...
ggplot(data = FB, aes(x = YDay, fill=as.factor(Wave))) + 
  ...
  ggtitle ("Number of observations per day number per year") +
...
FB %>%
  ...
  geom_col(aes(x = YDay, y = Prec_hours), fill = "darkblue", alpha = .6) +
  geom_point(aes(x = YDay, y = Hours_sun), color = "darkgoldenrod1", alpha = .6) +
  geom_smooth(aes (x = YDay, y = StepsK_mean, colour = as.factor(Wave)), alpha = .6, span = 1, method = 'lm', se = FALSE) +
  facet_grid(rows = vars(YearF)) +
  ggtitle ("Weather conditions per day number per year") +
  xlab("Day of the year") +  ylab ("") +
  labs(fill = "Wave")
...
This means that the conditional explained variance of these four levels is: .23.
```{r, echo=TRUE}
...
Moreover, we see positive effects of the temperature, the hours of sunshine and the humidity. Also a negative effect of the hours of rain per day. Let's make four graphs based on these results. First up, the effect of the mean temperature. The alpha of the points in the graph is set to the number of observations that is used to calculate the average score. So the higher the alpha (the more visible the dot), the more data is used. I have added a grey geom_smooth with a grey line and a red line. the red line is the regression coefficient the grey follows the data. This indicates that this effect is not en perfect linear function and seems to weaken after 20 degrees. Which makes sense, because then 
...
The effect of the hours of sunshine:
...
FB%>%
  ...
  xlab("uimidity") +  ylab ("1000 Steps") +
  theme(legend.position = "none")
```

In this plot you can observe that the relationship is probably not linear. Both very low and very high humid days predic less phsycial activity. The regression line is almost flat. 
...
So there you have it. The more it rains, the less active the kids in the MyMovez sample are. So is this a good excuse the skip practice for tonight?
